...
module Implementation
    def repeated_n_times(nums)
        non_repeteated_values = nums.group_by(&:itself).transform_values(&:count).select{ |k, v| v.eql? 1 }.keys
        repetead_values_with_count = nums.group_by(&:itself).transform_values(&:count).select{ |k, v| v != 1 }
